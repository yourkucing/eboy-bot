...
module.exports.run = async(client, msg, args) => {
    ...
    eboylog.send(`**${author.username}** [${author.id}] used the **withdraw** command in **${guild}** [${msg.guild.id}].`)
    ...
    if (!moneyData) {
        ...
        moneyModel.findOneAndUpdate({userID: hooman}, {
            ...
            if (!result) {
                ...
                errorlog.send(`<@279101053750870017>: Money cannot be withdrawn from bank. [User ID: ${author.id}, Guild ID: ${msg.guild.id}].`)
            }
            else {
                const embed = new MessageEmbed()
                .setColor('#FF69B4')
                .setDescription(`**${moneyData.bank} gp** has been withdrawn into your wallet.`);
